...
function [acc_fold, score_predict_repeat, label_predict_repeat] = classification_LR(data,label,subject,feat_ratio)
...
for r=1:repeat
    ...
    for f=1:fold
        ...
        model = glmfit(double(data_train),label_train,'binomial','link','logit');
        score_predict = glmval(model,double(data_test),'logit');
        score_predict = score_predict-0.5;
        label_predict = sign(score_predict);
        
        acc_fold(r,f)=100*sum(label_predict==label_test)/length(label_test);
        label_predict_all(f,indTest)=label_predict;
        score_predict_all(f,indTest)=score_predict;
    end
